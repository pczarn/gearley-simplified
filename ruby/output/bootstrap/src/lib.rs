...
enum Decl {
    ...
    RegexpTokenDecl { regexp: String, ident: String },
            ...
            ["#[derive(Clone)] pub struct RsTyGrammar {  }", "#[derive(Clone)] pub struct RsTyGrammar {  }", "#[derive(Clone)] pub struct RsTyRule {  }", "#[derive(Clone)] pub struct RsTyRule {  }", "#[derive(Clone)] pub struct RsTyNonterminalRule {  }", "#[derive(Clone)] pub struct RsTyTerminalRule {  }", "#[derive(Clone)] pub struct RsTyTerminalRule {  }", "#[derive(Clone)] pub struct RsTyGrammar {  }", "#[derive(Clone)] pub struct RsTyGrammar {  }"]
            type RsTyOpBnf = ();
type RsTyOpColon = ();
type RsTyLparen = ();
type RsTyRparen = ();
...
let sym_terminal_rule = grammar.make_symbol("terminal_rule");
                grammar.rule(sym_grammar).rhs([sym_grammar, sym_rule]).id(0).build();
grammar.rule(sym_grammar).rhs([sym_rule]).id(1).build();
grammar.rule(sym_rule).rhs([sym_nonterminal_rule]).id(2).build();
grammar.rule(sym_rule).rhs([sym_terminal_rule]).id(3).build();
grammar.rule(sym_nonterminal_rule).rhs([sym_ident, sym_op_bnf, sym_rule_rhs]).id(4).build();
grammar.rule(sym_terminal_rule).rhs([sym_literal_token_rule]).id(5).build();
grammar.rule(sym_terminal_rule).rhs([sym_regexp_token_rule]).id(6).build();
grammar.rule(sym_literal_token_rule).rhs([sym_quote, sym_string, sym_quote, sym_arrow, sym_ident]).id(7).build();
grammar.rule(sym_regexp_token_rule).rhs([sym_slash, sym_string, sym_slash, sym_arrow, sym_ident]).id(8).build();
grammar.rule(sym_string).rhs([sym_string, sym_any]).id(9).build();
grammar.rule(sym_string).rhs([sym_any]).id(10).build();
grammar.rule(sym_ident).rhs([sym_alpha, sym_alnum_str]).id(11).build();
grammar.rule(sym_alnum_str).rhs([sym_alnum_str, sym_alnum]).id(12).build();
grammar.rule(sym_alnum_str).rhs([sym_alnum]).id(13).build();
grammar.rule(sym_grammar).rhs([sym_grammar, sym_rule]).id(14).build();
...
4 => {
                ...
                Value::nonterminal_rule({ RsTyNonterminalRule {  } })
            }
5 => {
                
                Value::terminal_rule({ RsTyTerminalRule {  } })
            }
6 => {
...
7 => {
                let mut string = match args[1].clone() { Value::string(val) => val, _ => panic!("wrong sym") };
let mut ident = match args[4].clone() { Value::ident(val) => val, _ => panic!("wrong sym") };
                Value::literal_token_rule({  Decl::LiteralTokenDecl { string, ident }  })
            }
8 => {
                let mut string = match args[1].clone() { Value::string(val) => val, _ => panic!("wrong sym") };
let mut ident = match args[4].clone() { Value::ident(val) => val, _ => panic!("wrong sym") };
                Value::regexp_token_rule({  Decl::RegexpTokenDecl { regexp: string, ident }  })
            }
9 => {
                let mut s = match args[0].clone() { Value::string(val) => val, _ => panic!("wrong sym") };
let mut ch = match args[1].clone() { Value::any(val) => val, _ => panic!("wrong sym") };
                Value::string({  s.push(ch); s  })
            }
10 => {
                let mut ch = match args[0].clone() { Value::any(val) => val, _ => panic!("wrong sym") };
                Value::string({  ch.to_string()  })
            }
11 => {
                let mut ch = match args[0].clone() { Value::alpha(val) => val, _ => panic!("wrong sym") };
let mut s = match args[1].clone() { Value::alnum_str(val) => val, _ => panic!("wrong sym") };
                Value::ident({  s.push(ch); s  })
            }
12 => {
                let mut s = match args[0].clone() { Value::alnum_str(val) => val, _ => panic!("wrong sym") };
let mut ch = match args[1].clone() { Value::alnum(val) => val, _ => panic!("wrong sym") };
                Value::alnum_str({  s.push(ch); s  })
            }
13 => {
                let mut ch = match args[0].clone() { Value::alnum(val) => val, _ => panic!("wrong sym") };
...
15 => {
                ...
                let terminal_eval = |terminal, values| {
                    ...
                    if terminal == grammar.sym("op_bnf").unwrap() { Value::op_bnf(if ordinal == 0 {  } else { unreachable!() }) } else if terminal == grammar.sym("op_colon").unwrap() { Value::op_colon(if ordinal == 0 {  } else { unreachable!() }) } else if terminal == grammar.sym("lparen").unwrap() { Value::lparen(if ordinal == 0 {  } else { unreachable!() }) } else if terminal == grammar.sym("rparen").unwrap() { Value::rparen(if ordinal == 0 {  } else { unreachable!() }) } else if terminal == grammar.sym("op_plus").unwrap() { Value::op_plus(if ordinal == 0 {  } else { unreachable!() }) } else if terminal == grammar.sym("op_star").unwrap() { Value::op_star(if ordinal == 0 {  } else { unreachable!() }) } else if terminal == grammar.sym("quote").unwrap() { Value::quote(if ordinal == 0 {  } else { unreachable!() }) } else if terminal == grammar.sym("doublequote").unwrap() { Value::doublequote(if ordinal == 0 {  } else { unreachable!() }) } else if terminal == grammar.sym("slash").unwrap() { Value::slash(if ordinal == 0 {  } else { unreachable!() }) } else if terminal == grammar.sym("arrow").unwrap() { Value::arrow(if ordinal == 0 {  } else { unreachable!() }) } else if terminal == grammar.sym("alpha").unwrap() { Value::alpha(if ordinal == 0 {  slice.chars().next().unwrap()  } else { unreachable!() }) } else if terminal == grammar.sym("alnum").unwrap() { Value::alnum(if ordinal == 0 {  slice.chars().next().unwrap()  } else { unreachable!() }) } else if terminal == grammar.sym("any").unwrap() { Value::any(if ordinal == 0 {  slice.chars().next().unwrap()  } else { unreachable!() }) }
                    else {
                        Value::None
                    }
                };
