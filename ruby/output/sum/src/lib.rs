            ...
            ["#[derive(Clone)] pub struct RsTySum {  }", "#[derive(Clone)] pub struct RsTyFactor {  }", "#[derive(Clone)] pub struct RsTyFactor {  }", "#[derive(Clone)] pub struct RsTyFactor {  }", "#[derive(Clone)] pub struct RsTyExpr {  }", "#[derive(Clone)] pub struct RsTyExpr {  }", "#[derive(Clone)] pub struct RsTyExpr {  }", "#[derive(Clone)] pub struct RsTyNumber {  }", "#[derive(Clone)] pub struct RsTyNumber {  }", "#[derive(Clone)] pub struct RsTyWhole {  }", "#[derive(Clone)] pub struct RsTyWhole {  }"]
            type RsTyLparen = ();
type RsTyRparen = ();
type RsTyOpMinus = ();
type RsTyOpPlus = ();
...
let sym_whole = grammar.make_symbol("whole");
                grammar.rule(sym_sum).rhs([sym_sum, sym_op_plus, sym_factor]).id(0).build();
grammar.rule(sym_sum).rhs([sym_sum, sym_op_minus, sym_factor]).id(1).build();
grammar.rule(sym_sum).rhs([sym_factor]).id(2).build();
grammar.rule(sym_factor).rhs([sym_factor, sym_op_mul, sym_expr]).id(3).build();
grammar.rule(sym_factor).rhs([sym_factor, sym_op_div, sym_expr]).id(4).build();
grammar.rule(sym_factor).rhs([sym_expr]).id(5).build();
grammar.rule(sym_expr).rhs([sym_lparen, sym_sum, sym_rparen]).id(6).build();
grammar.rule(sym_expr).rhs([sym_op_minus, sym_sum]).id(7).build();
grammar.rule(sym_expr).rhs([sym_number]).id(8).build();
grammar.rule(sym_number).rhs([sym_whole]).id(9).build();
grammar.rule(sym_number).rhs([sym_whole, sym_dot, sym_whole]).id(10).build();
grammar.rule(sym_whole).rhs([sym_whole, sym_digit]).id(11).build();
...
Token::Dot(ordinal) => (grammar.sym("dot").unwrap(), ordinal),
                ...
                let rule_eval = |rule_id, args: &[Value]| {
                    match rule_id {
                        0 => {
                let mut left = match args[0].clone() { Value::sum(val) => val, _ => panic!("wrong sym") };
let mut right = match args[2].clone() { Value::factor(val) => val, _ => panic!("wrong sym") };
...
12 => {
                ...
                let terminal_eval = |terminal, values| {
                    ...
                    if terminal == grammar.sym("lparen").unwrap() { Value::lparen(if ordinal == 0 {  } else { unreachable!() }) } else if terminal == grammar.sym("rparen").unwrap() { Value::rparen(if ordinal == 0 {  } else { unreachable!() }) } else if terminal == grammar.sym("op_minus").unwrap() { Value::op_minus(if ordinal == 0 {  } else { unreachable!() }) } else if terminal == grammar.sym("op_plus").unwrap() { Value::op_plus(if ordinal == 0 {  } else { unreachable!() }) } else if terminal == grammar.sym("op_mul").unwrap() { Value::op_mul(if ordinal == 0 {  } else { unreachable!() }) } else if terminal == grammar.sym("op_div").unwrap() { Value::op_div(if ordinal == 0 {  } else { unreachable!() }) } else if terminal == grammar.sym("digit").unwrap() { Value::digit(if ordinal == 0 {  } else { unreachable!() }) } else if terminal == grammar.sym("dot").unwrap() { Value::dot(if ordinal == 0 {  } else { unreachable!() }) }
                    else {
                        Value::None
                    }
                };
